...
export default function StaffHomePage() {
  ...
  // Event staff accounts are minted with `department` equal to the event's type
  // (`departmentForEvent` — e.g. a Last1Standing volunteer in `sports`). That
  // would otherwise qualify them as that domain's admin via `isDomainAdminFor`
  // below and list every other event of that type here: Sprintsaga, Kampus Run,
  // and the rest, none of which they run. Their own roster already lives in the
  // duty section beneath the masonry. So "Participation & Reports" is withheld
  // from anybody on an event team — Event Head, Event Member, or Volunteer —
  // not only from heads. It stays for a UHC officer, domain admin, or Super
  // Admin who is not also staffing an event.
  const participationEvents =
    myEventTeams.length > 0
      ? []
      : events.filter((e) => isUhc() || isDomainAdminFor(e.event_type) || isSuperAdmin());
