        ...
        <>
          ...
          <Card className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <p className="font-semibold text-gray-900">Mess fee</p>
              {payments.mess && (
                <span
      ...
      <div className="flex items-center justify-between">
        ...
        <span
          className={`rounded-full px-2 py-0.5 text-xs font-medium ${
            payment ? STATUS_BADGE[payment.status] : 'bg-gray-100 text-gray-600'
          }`}
        >
  ...
  return (
    <div className="rounded-lg bg-gray-50 p-3 text-sm">
      ...
      <div className="mt-1 flex justify-between text-muted">
        <span>Amount</span>
        <span className="text-gray-900">{money(payment.amount, payment.currency)}</span>
      </div>
      {payment.planName && (
        <div className="flex justify-between text-muted">
          <span>Plan</span>
          <span className="text-gray-900">{payment.planName}</span>
        </div>
      )}
      <div className="flex justify-between text-muted">
        <span>Date</span>
        <span className="text-gray-900">{payment.paidAt?.slice(0, 10) ?? '—'}</span>
      </div>
      <div className="flex justify-between text-muted">
        <span>Ref</span>
        <span className="text-gray-900">{payment.txnRef ?? '—'}</span>
      </div>
    </div>
  );
}
