      ...
      <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-brand to-brand-dark p-5 shadow-lift">
        {status === 'ready' && code && participant && (
          <div className="flex items-center gap-3 pb-4 text-white">
            <Avatar src={participant.photoUrl} name={participant.fullName || participant.email} size={40} />
            <div className="min-w-0">
              <p className="truncate font-semibold">{participant.fullName || participant.email}</p>
              <p className="truncate text-xs text-white/70">{CHECKPOINT_LABELS[checkpoint]} · ID {participant.id}</p>
            </div>
          </div>
        )}
  ...
  return (
    <div className="relative h-12 w-12">
      <svg viewBox="0 0 36 36" className="h-12 w-12 -rotate-90">
        <circle cx="18" cy="18" r={radius} fill="none" stroke="currentColor" className="text-surface-2" strokeWidth="3" />
        <circle
          cx="18"
          cy="18"
          r={radius}
